...
test('renderWorldCupPacket uses customer-facing forecast language and no raw UTC headers', () => {
  ...
  assert.match(text, /Argentina vs Austria: Argentina favored; projected goals 1\.79-0\.94; projected total 2\.73/);
  assert.match(text, /Status: Lineup pending, lineups not confirmed/);
  ...
  assert.match(text, /Forecast context: lineups are not confirmed yet\./);
  assert.match(text, /First-half views are unavailable because no split-layer model is sourced\./);
  assert.match(text, /Perplexity research: live supplemental context captured/);
  assert.doesNotMatch(text, /\b(?:PICK|LEAN|WATCH|FADE|winner_lean|projection-only|actionable|monitor|top edge candidates|trigger board|overpriced|market|price|odds|display-only|NOT IN SCORE)\b/i);
  ...
  assert.match(text, /Research only\./);
});
