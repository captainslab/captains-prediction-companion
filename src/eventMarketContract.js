...
// Price isolation: posture must not depend on market price, the fair-vs-price
// edge, bid/ask, or last_price. Direction comes from the model's OWN probability
// (fair_yes, our composite — not market data); conviction comes from the model's
// confidence. Market price never enters this decision.
function evidenceThresholdRecommendation(marketStatus, fairYes, confidence) {
  if (marketStatus && marketStatus !== 'active') return 'pass';
  if (fairYes == null) return 'watch';
  const conviction = normalizeConfidence(confidence);
  if (conviction !== 'high' && conviction !== 'medium') return 'watch';
  return fairYes >= 0.5 ? 'buy_yes' : 'buy_no';
}
...
function buildUserFacingMarketView(input, marketType, eventType) {
  ...
  if (marketType === 'mention') {
    ...
    const alphaConfidence = metadataValue(metadata, 'alpha_confidence');
    const availableContracts = Array.isArray(metadata.available_contracts) ? metadata.available_contracts : [];
    // marketYes / edgeCents are read only to echo into trade_view for display below;
    // they are intentionally NOT passed into posture.
    const bestSide = evidenceThresholdRecommendation(marketStatus, fairYes, alphaConfidence);
...
function buildUserFacingStatus(eventType, marketType, marketView) {
  ...
  if (marketType === 'mention') {
    // Readiness is gated on the model's own fair-value read existing, not on any
    // price/edge field. market_yes presence only marks that live pricing exists.
    if (marketView.trade_view?.fair_yes != null) {
      return 'ready';
    }
...
function buildUserFacingReason(status, marketType, input, marketView) {
  ...
  if (status === 'ready' && marketType === 'mention') {
    if (marketView?.trade_view?.best_side === 'watch') {
      return (
        alphaReason ??
        'The model read stays on WATCH until stronger source evidence or model conviction appears.'
      );
    }
    return (
      alphaReason ??
