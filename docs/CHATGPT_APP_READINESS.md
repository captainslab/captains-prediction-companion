...
Default MCP tool responses are card-first and concise. ChatGPT should see a short human-readable summary plus structuredContent with the app-safe card fields, not raw packet text or internal workflow objects.
...
- [ ] `app_status` includes `appName`, `endpoint`, `toolList`, `readOnlyDefault`, `noteToolsEnabled`, and no-trade/no-send/no-public-write posture.
...
- [ ] `structuredContent` prefers `title`, `plain_english`, `settlement`, `route`, `cpc_read`, `model_read`, `evidence_status`, `base_rate`, `price_context`, `ticker`/`market_id`, `source_summary`, and `blocked_reason` when blocked.
- [ ] Ticker or market ID is secondary, not the headline.
- [ ] Price context says display-only and not used in scoring.
- [ ] No banned customer-facing language appears in the app-facing content.
- [ ] No local filesystem paths appear in the app-facing content.
